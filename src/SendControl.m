...
- (id)initWithSendMessage:(NSString*)msg recvMessage:(RecvMessage*)recv {
    ...
    [userTable setDoubleAction:@selector(userTableDbClick:)];

	return self;
}

//访问单实例
+ (SendControl*) shareSendControl
{
    static SendControl* share = nil;
    if (!share) {
        share = [[SendControl alloc] initWithSendMessage:nil recvMessage:nil];
        return share;
    }
    else
        return share;
...
- (IBAction)sendPressed:(id)sender {
    ...
    ReceiveControl* receiveWin = [[WindowManager sharedManager] receiveWindowForKey:touser.hostName];
    if (receiveWin) {
        NSMutableString *replytxt = [[NSMutableString alloc] init];
        [replytxt appendFormat:@"\r%@:\r    %@\r",MY_NICKNAME,msg];
        NSMutableAttributedString* attrtxt = [[NSMutableAttributedString alloc] initWithString:replytxt];
        [attrtxt addAttribute:NSForegroundColorAttributeName
                            value:[NSColor blueColor]
                            range:NSMakeRange(0, [attrtxt length])];
        [[receiveWin messageArea ] setEditable:YES];
        [[receiveWin messageArea ] insertText:attrtxt];
        [[receiveWin messageArea ] setEditable:NO];
    }
...
- (void)userTableDbClick:(id)sender {
    NSInteger row = [userTable clickedRow];
    if (row >= 0) {
        UserInfo* info = [users objectAtIndex:row];
        ReceiveControl*	recv;
        if (info && info.hostName) {
            recv = [[WindowManager sharedManager] receiveWindowForKey:[info hostName]];
            if (!recv) {
                recv = [[ReceiveControl alloc] initWithRecvMessage:info];
            }
            [recv showWindow];
        }
    }
