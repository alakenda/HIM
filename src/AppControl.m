...
- (IBAction)newMessage:(id)sender {
    ...
    SendControl* sendctl = [SendControl shareSendControl];
    [[sendctl window] makeKeyAndOrderFront:nil];
}
...
- (void)receiveMessage:(RecvMessage*)msg {
	...
	// 受信ウィンドウ生成（まだ表示しない）
    recv = [[WindowManager sharedManager] receiveWindowForKey:[[msg fromUser] hostName]];
    if (!recv) {
        recv = [[ReceiveControl alloc] initWithRecvMessage:[msg fromUser]];
    }

    [recv updateRecvMessage:msg];

	if (config.nonPopup) {
		if ((config.nonPopupWhenAbsence && config.inAbsence) ||
			(!config.nonPopupWhenAbsence)) {
			// ノンポップアップの場合受信キューに追加
            BOOL isActive = recv.window.isKeyWindow;
			...
			switch (config.iconBoundModeInNonPopup) {
                case IPMSG_BOUND_ONECE:
                    ...
                    if (![[NSApplication sharedApplication] isActive])
                        [[MessageCenter sharedCenter] showMsgInNC:msg];
                    break;
