...
- (id)initWithBuffer:(const void*)buf length:(NSUInteger)len from:(struct sockaddr_in)addr
{
    ...
    if (hostName == nil || [hostName isEqualToString:@""]) {
        NSLog(@"invalid hostname,pakage no:%ld,username:%@",(long)_packetNo,logOnUser);
    }
	...
	if (message) {
		if (command & IPMSG_UTF8OPT) {
			_appendix = [[NSString alloc] initWithUTF8String:message];
		} else {
			_appendix = [[NSString alloc] initWithSJISString:message];
		}
	}
	...
	switch (GET_MODE(command)) {
        ...
        case IPMSG_SENDSCREENSHOT:
            _bmpHeader = [self readScreenShotPkgHeader];
            if ((tok = strtok_r(NULL, MESSAGE_FEIQEXT_SEPARATOR, &ptr))) {
                //ptr+1是因为#后面的\x00不是bmp数据,而是上一个头字符串的结尾.
                _binData = [NSData dataWithBytes:ptr + 1 length:self.bmpHeader.pkgLen];
            }
            break;
	}
	...
	case IPMSG_BR_ABSENCE:
		[fromUser release];
		fromUser = [[UserInfo alloc] initWithUserName:_appendix
		...
		if ((command & IPMSG_FILEATTACHOPT) && subMessage) {
			...
			if ([msgs count] > 0) {
				...
				for (NSString* str in msgs) {
                    ...
                    Attachment* attach= [Attachment attachmentWithMessage:str pkgNo:self.packetNo];
...
- (NSString*)description {
	return [NSString stringWithFormat:@"RecvMessage:command=0x%08lX,PacketNo=%ld,from=%@", command, (long)self.packetNo, fromUser];
}
...
- (id)copyWithZone:(NSZone*)zone {
	...
	if (newObj) {
		...
		newObj->_appendix		= [_appendix retain];
...
//获取当前数据包中的截屏数据包头
-(ScreenShotPkgHeader * ) readScreenShotPkgHeader;
{
    NSRange range = [_appendix rangeOfString:@"#"];
    if (range.length != NSNotFound) {
        NSString* pkgHeader = [_appendix substringToIndex:range.location -1];
        
        NSArray* strArray = [pkgHeader componentsSeparatedByString:@"|"];
        ScreenShotPkgHeader* header = [[ScreenShotPkgHeader alloc] init];
        header.bmpName = [strArray objectAtIndex:0];
        header.totoalByte = [[strArray objectAtIndex:1] intValue];
        header.startByte = [[strArray objectAtIndex:2] intValue];
        header.totalPkg = [[strArray objectAtIndex:3] intValue];
        header.curPkgNo = [[strArray objectAtIndex:4] intValue];
        header.pkgLen = [[strArray objectAtIndex:5] intValue];
        header.flag1 = [[strArray objectAtIndex:6] intValue];
        header.flag2 = [[strArray objectAtIndex:7] intValue];
        header.flag3 = [[strArray objectAtIndex:8] intValue];
        return header;
    }
    else
        return nil;
