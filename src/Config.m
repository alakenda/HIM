...
static NSString* GEN_MAC_ADDR           = @"BindMacAddress";
...
@synthesize emotionImgDic               = _emotionImgDic;
...
- (id)init
{
	...
	[mutableDic setObject:[NSNumber numberWithBool:YES] forKey:GEN_USE_STATUS_BAR];
    [mutableDic setObject:@"" forKey:GEN_MAC_ADDR];
	// ネットワーク
	...
	[mutableDic setObject:[NSNumber numberWithBool:YES] forKey:RECV_NON_POPUP];
	...
	[mutableDic setObject:FILE_STD_LOG forKey:LOG_STD_FILE];
	...
	[mutableDic setObject:FILE_ALT_LOG forKey:LOG_ALT_FILE];
	// 送信ウィンドウ
	...
	[mutableDic setObject:[NSNumber numberWithBool:YES] forKey:SNDSEARCH_HOST];
	[mutableDic setObject:[NSNumber numberWithBool:YES] forKey:SNDSEARCH_LOGON];
    ...
    //获取mac地址
    self.macaddress			= [defaults stringForKey:GEN_MAC_ADDR];
    if (self.macaddress	== nil || [self.macaddress length] < 12) {
        self.macaddress = [self getMacAddress];
    }
    //
    //NSString* macaddr =
    NSLog(@"Mac addr:%@",self.macaddress);
	...
	self.sendSearchByHostName		= YES;//[defaults boolForKey:SNDSEARCH_HOST];
	self.sendSearchByLogOnName		= YES;//[defaults boolForKey:SNDSEARCH_LOGON];
    ...
    //edit by heqian
    //初始化表情字典
    NSURL* url = [[NSBundle mainBundle] URLForResource:@"emotionstr" withExtension:@"plist"];
    self.emotionImgDic = [[NSDictionary alloc] initWithContentsOfURL:url];
    
    //初始化日志目录,截屏图片目录
    NSFileManager* fileManager = [NSFileManager defaultManager];
    if (![fileManager fileExistsAtPath:[DIR_LOG stringByExpandingTildeInPath]]) {
		if (![fileManager createDirectoryAtPath:[DIR_LOG stringByExpandingTildeInPath] withIntermediateDirectories:YES attributes:nil error:nil]) {
			ERR(@"LogFileDir Create Error.(%@)", [DIR_LOG stringByExpandingTildeInPath]);
			return;
		}
	}
    if (![fileManager fileExistsAtPath:[DIR_Rcv_Photo stringByExpandingTildeInPath]]) {
		if (![fileManager createDirectoryAtPath:[DIR_Rcv_Photo stringByExpandingTildeInPath] withIntermediateDirectories:YES attributes:nil error:nil]) {
			ERR(@"LogFileDir Create Error.(%@)", [DIR_Rcv_Photo stringByExpandingTildeInPath]);
			return;
		}
	}
...
- (void)save
{
    ...
    //[def setObject:self.macaddress forKey:GEN_MAC_ADDR];
...
- (void)insertAbsenceTitle:(NSString*)title message:(NSString*)msg atIndex:(NSUInteger)index
{
	@try {
		...
		ERR(@"%@(title=%@,msg=%@,index=%lu)", exception, title, msg, (unsigned long)index);
	}
}
...
- (void)setAbsenceTitle:(NSString*)title message:(NSString*)msg atIndex:(NSInteger)index
{
	@try {
		...
		ERR(@"%@(title=%@,msg=%@,index=%ld)", exception, title, msg, (long)index);
	}
}
